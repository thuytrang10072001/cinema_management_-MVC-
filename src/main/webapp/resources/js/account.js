(function($) {
	...
	/*------ ScrollUp -------- */
	$.scrollUp({
		scrollText: '<i class="fa-regular fa-up-from-line"></i>',
		easingType: "linear",
		scrollSpeed: 900,
		animation: "fade",
	});
	...
	$(".hero-slider-1").slick({
		...
		arrows: true,
		prevArrow:
			'<span class="slider-btn slider-prev"><i class="fa-regular fa-angle-left"></i></span>',
		nextArrow:
			'<span class="slider-btn slider-next"><i class="fa-regular fa-angle-right"></i></span>',
	...
	$(function() {
		sliderrange.slider({
			...
			slide: function(event, ui) {
				...
				// amountprice.val(
				//   ui.values[0] + "000 VND" + " - " + ui.values[1] + "000 VND",
				// );
			},
		});
	...
	/*---------------------
		  Reviews range
	  --------------------- */
	var reviewRange = $("#review-range");
	// var amountprice = $("#amount");
	var reviewStart = $("#review-start");
	var reviewEnd = $("#review-end");
	$(function() {
		reviewRange.slider({
			range: true,
			min: 0,
			...
			step: 0.1,
			slide: function(event, ui) {
				reviewStart.val(ui.values[0]);
				reviewEnd.val(ui.values[1]);
			},
		});
		reviewStart.val(reviewRange.slider("values", 0));
		reviewEnd.val(reviewRange.slider("values", 1));
	});
	/*-------------------------------
		  Sort by active
	  -----------------------------------*/
	...
	// edit info receiver
	let btnEditInfoReceiver = $(".edit-info-receiver");
	let btnSaveInfoReceiver = $(".save-edit-info-receiver");
	let btnCancelInfoReceiver = $(".cancel-edit-info-receiver");
	let inputReceiverName = $(".input-receiver-name");
	let inputReceiverPhoneNumber = $(".input-receiver-phone-number");
	let inputReceiverAddress = $(".input-receiver-address");
	...
	btnSaveInfoReceiver.on("click", function() {
		...
		$(".info-receiver-content").removeClass("d-none");
		receiverName.text(inputReceiverName.val());
		receiverPhoneNumber.text(inputReceiverPhoneNumber.val());
		receiverAddress.text(inputReceiverAddress.val());
	});
	btnCancelInfoReceiver.on("click", function() {
		$(".form-info-receiver").addClass("d-none");
		$(".info-receiver-content").removeClass("d-none");
	});
